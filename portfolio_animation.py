...
# --- Plotly Chart 2: Efficient Frontier (Scatter Plot) ---
print("\n--- Generating Plotly Chart 2: Efficient Frontier ---")
...
fig = px.scatter(efficient_frontier_df, x='Annualized Volatility', y='Annualized Mean Return',
                 color='Portfolio Type',
                 title='Efficient Frontier',
                 hover_data={'Annualized Volatility': True, 'Annualized Mean Return': True})
...
# Manually calculate risk contributions using weights and covariance
# Risk contribution = w_i * (Cov * w)_i / (w' * Cov * w)
risk_contributions_data = []

# Mean-Risk Portfolio
weights_mr = mean_risk_model.weights_
covariance_mr = X_train.cov()
# dot product of covariance matrix and weights
cov_weights_product_mr = np.dot(covariance_mr, weights_mr)
# total risk of the portfolio (w' * Cov * w)
